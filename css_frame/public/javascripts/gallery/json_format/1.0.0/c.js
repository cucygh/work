...
function Process(){
  ...
  var json = $id("res-param").value;
  var html = "";
  try{
    if(json == "") json = "\"\"";
    var obj = eval("["+json+"]");
    html = ProcessObject(obj[0], 0, false, false, false);
    $id("json-formated").innerHTML = "<h4>服务器响应格式化：</h4><PRE class='CodeContainer'>"+html+"</PRE>";
  }catch(e){
    alert("JSON数据格式不正确:\n"+e.message);
    $id("json-formated").innerHTML = "";
  }
}
...
function CollapseAllClicked(){
  ...
  TraverseChildren($id("json-formated"), function(element){
...
function CollapseLevel(level){
  ...
  TraverseChildren($id("json-formated"), function(element, depth){
...
function EnsureIsPopulated(){
  ...
  if(!$id("json-formated").innerHTML && !!$id("res-param").value) Process();
...
function SelectAllClicked(){
  ...
  if(!!range.selectNode)
    ...
    range.selectNode($id("json-formated"));

  else if(range.moveToElementText)

    range.moveToElementText($id("json-formated"));

  

  if(!!range.select)

    range.select($id("json-formated"));
